...
class NeutralinoExtension:
    def __init__(self, debug=False):
        ...
        if len(sys.argv) > 1:
            ...
            self.connectToken = ''
            self.urlSocket = f"ws://localhost:{self.port}?extensionId={self.idExtension}"
            self.callback = None
        else:
            ...
            self.urlSocket = f"ws://localhost:{self.port}?extensionId={self.idExtension}&connectToken={self.connectToken}"
